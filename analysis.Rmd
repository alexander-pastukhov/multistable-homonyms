...
none_reponses_across_time <- 
  percepts %>%
  group_by(Condition, Participant, Percept, word, Time) %>%
  summarize(Proportion = 100 * sum(Duration) / BlockDuration[1], 
            .groups="drop") %>%
  filter(Condition != "Test", Percept == 0) %>%
  mutate(PerceptF = factor(Percept, 
                           levels= c(0),
                           labels = c("None")))

ggplot(none_reponses_across_time, aes(x = Time, y = Proportion)) +
  geom_line() +
  scale_x_continuous("Time [s]", limits = c(0, 75)) +
  scale_y_continuous("Proportion of None responses", limits = c(0, 100))
```
