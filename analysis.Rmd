...
none_through_time_df <-
  ...
  # share common time limit
  mutate(LongestBlock = max(BlockDuration)) |>
...
none_through_time_avg <-
  none_through_time_df |>
  group_by(Participant, Time) |>
  summarize(avgNoneResponse = mean(IsNone)) |>
  group_by(Time) |>
  summarize(avgNone = mean(avgNoneResponse))

# compute group average per time for bootstrapped data 1000 times
sample_group_average <- function(df, replace=TRUE) {
  df |>
    group_by(Participant, Time) |>
    slice_sample(prop = 1, replace = replace) |>
    summarise(avgNoneResponse = sum(IsNone)  / n(), 
              .groups = "drop") |>
    group_by(Time) |>
    summarise(avgNone = mean(avgNoneResponse))
}

sampled_avgs <- purrr::map_dfr(1:1000, ~sample_group_average(none_through_time_df), .progress = TRUE)

# compute percentile CIs per time
bootstrapped_CIs <-
  sampled_avgs %>%
  group_by(Time) %>%
  summarise(LowerCI = quantile(avgNone, (1 - 0.97) / 2),
            UpperCI = quantile(avgNone, 1 - (1 - 0.97) / 2),
            .groups = "drop")

# plot group averages and CIs
group_avgs <- left_join(none_through_time_avg, bootstrapped_CIs, by = "Time")


ggplot(group_avgs, aes(x = Time, y = avgNone)) + 
  geom_ribbon(aes(ymin = LowerCI, ymax = UpperCI), alpha = 0.5) +
  geom_line() +
  scale_y_continuous(name = "Average proportion of None responses", limits = c(0, 1)) +
