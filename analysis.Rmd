...
none_reponses <- 
  percept_summary %>%
  filter(Percept == 0, Condition != "Test") %>%
  mutate(PerceptF = factor(Percept, 
                           levels= c(0),
                           labels = c("None"))) %>%
  complete(Participant, word, PerceptF, fill=list(Proportion=0, TotalDuration=0, N=0))

none_responses_plot <- ggplot(none_reponses, aes(x = reorder(word, -Proportion), y = Proportion)) +
  geom_boxplot(outlier.shape = NA) +
  geom_quasirandom(method="tukeyDense", size=0.7) +
  xlab("") +
  scale_y_continuous(name = "Proportion of None responses [%]", limits = c(0, 100)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
none_responses_plot
```
...
### Across time
```{r}
none_responses_across_time <- 
  percepts %>%
  filter(Participant == "UDA1993F", word == "Bar")

time_series_data <- 
  tibble(
    Participant = none_responses_across_time$Participant[1],
    Word = none_responses_across_time$word[1],
    Seconds = 1:ceiling(none_responses_across_time$BlockDuration),
    Percept = NA)

for (t in 1:length(none_responses_across_time$Time)) {
  time_series_data$Percept = ifelse(time_series_data$Seconds >= none_responses_across_time$Time[t], none_responses_across_time$Percept[t], time_series_data$Percept)
}
```
...
time_series_function <- function(df){
  data <- tibble(
    ...
    Word = df$word[1],
    Seconds = 1:ceiling(df$BlockDuration),
    Percept = NA)
  
 for (t in 1:length(df$Time)) {
  data$Percept = ifelse(data$Seconds >= df$Time[t], df$Percept[t], data$Percept)
 }
  
  return(data)
}

single_participant <- time_series_function(none_responses_across_time)

time_series_data_complete <- 
  percepts %>%
  group_by(Participant, word) %>%
  group_map(~time_series_function(.x))
```

### Sasha's version

```{r}
compute_none_through_time <- function(df){
  none_in_time_df <- tibble(
    Time = 0:ceiling(df$BlockDuration),
    IsNone = FALSE
  )
  for(irow in 1:nrow(df)) {
    # we find ALL time points AFTER the onset of the word and mark ALL of them based on the current percept
    # if there is a percept after that, it will overwrite relevant portion of the time series later on
    iafter <- none_in_time_df$Time >= df$Time[irow]
    none_in_time_df$IsNone[iafter] <- df$Percept[irow] == -1
  }
  
  none_in_time_df
}

none_through_time_df <-
  percepts |>
  
  # compute raw for each participant and word
  group_by(Participant, word) |>
  group_modify(~compute_none_through_time(.))
  

# compute participant average (per time) and then group average (per time)

# do the same thing for bootstrapped data 1000 times to compute group CIs

# compute percentile CIs (per time)

# plot group averages and CIs
```
...
```{r}
(uniplot / biplot / triplot / none_responses_plot) | (balance_plot / timeseriesplot)  +
