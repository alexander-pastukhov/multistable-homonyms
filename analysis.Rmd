...
### Across time
```{r}
none_reponses_across_time <- 
  percepts %>%
  filter(Participant == "UDA1993F", word == "Bar")

time_series_data <- 
  tibble(
    Participant = none_reponses_across_time$Participant[1],
    Word = none_reponses_across_time$word[1],
    Seconds = 1:ceiling(none_reponses_across_time$BlockDuration),
    Percept = NA)

for (t in 1:length(none_reponses_across_time$Time)) {
  time_series_data$Percept = ifelse(time_series_data$Seconds >= none_reponses_across_time$Time[t], none_reponses_across_time$Percept[t], time_series_data$Percept)
}
```
